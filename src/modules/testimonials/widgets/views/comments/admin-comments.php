...
/* @var $items \common\modules\testimonials\widgets\CommentView[] */
/* @var $count integer */
/* @var $commentForm \common\modules\testimonials\forms\TestimonialsForm */
?>

<div id="comments" class="inner-bottom-xs">
    <h2>Comments</h2>
    <?php foreach ($items as $item): ?>
        <?= $this->render('_admin-comment', ['item' => $item,'model'=>$model]) ?>
    <?php endforeach; ?>
</div>

<div id="reply-block" class="leave-reply">
    <?php $form = ActiveForm::begin([
        'action' => \yii\helpers\Url::to(['comment', 'id' => $model->id]),
    ]); ?>
...
<?php $this->registerJs("
    jQuery(document).on('click', '#comments .comment-reply', function () {
        ...
        jQuery('#testimonialsform-parentid').val(comment.data('id'));
